...
apply_defaults() {
	...
	: "${API_COMMANDS_STREAM_PATH_TEMPLATE:=/devices/{device_id}/commands/stream}"
	: "${HEARTBEAT_INTERVAL_SECONDS:=15}"
	: "${UPLOADER_POLL_SECONDS:=3}"
	: "${UPLOADER_MAX_RETRIES:=5}"
	: "${COMMAND_WS_ENABLED:=false}"
	: "${COMMAND_POLL_FALLBACK_ENABLED:=false}"
	: "${COMMAND_WS_CONNECT_TIMEOUT_SECONDS:=15}"
	: "${COMMAND_WS_HEARTBEAT_INTERVAL_SECONDS:=30}"
	: "${COMMAND_WS_BACKOFF_MAX_SECONDS:=30}"
}
